...
/***/function (module, exports, __webpack_require__) {
	...
	function drawgrid() {
		...
		for (var x = 0; x < global.screenWidth; x += global.screenHeight / 18) {
			graph.moveTo(x, 0);
			graph.lineTo(x, global.screenHeight);
			cols++;
		}

		for (var y = 0; y < global.screenHeight; y += global.screenHeight / 18) {
			graph.moveTo(0, y);
	...
	var grid = [];
	for (var ii = 0; ii < global.rows; ii++) {
		grid.push([]);
		for (var jj = 0; jj < global.columns; jj++) {
			grid[ii].push(undefined);
		}
	}
